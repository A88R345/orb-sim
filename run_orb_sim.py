"""
run_orb_sim.py — Orchestrateur, version alignée sur la logique validée
(orb_signal.py corrigé, T2=25 exact). Idempotent : rejouable sans jamais
dupliquer une ligne ni renvoyer deux fois la même notification.
"""
...
COLUMNS = [
    ...
    "or_low", "or_high", "bias_slope", "contracts", "exit_time", "exit_price",
...
def events_to_rows(events: list[TradeEvent], date_str: str) -> list[dict]:
    ...
    for e in events:
        rows.append({
            "date": date_str, "trade_id": e.trade_id, "direction": e.direction,
            "entry_time": e.entry_time, "entry_price": round(e.entry_price, 2),
            "sl": round(e.sl, 2), "tp": round(e.tp, 2),
            "or_low": round(e.or_low, 2), "or_high": round(e.or_high, 2),
            "bias_slope": round(e.bias_slope, 4), "contracts": e.contracts,
...
def notify_entry(r: dict) -> None:
    notify(
        ...
        f"Entrée : {r['entry_price']} | SL : {r['sl']} | TP : {r['tp']} | {r['contracts']} contrats\n"
...
def notify_exit(r: dict) -> None:
    ...
    notify(
        ...
        f"Sortie : {r['exit_price']} | PnL : {r['pnl_pts']} pts ({sign}{r['pnl_usd']}$ sur MNQ)"
    )


def main(today: pd.Timestamp | None = None) -> None:
    if today is None:
